...
function startPolling() {
    ...
    console.log('[Polling] Iniciado - atualização a cada 10 segundos');

    // Polling para lista principal de turmas (10 segundos)
    pollingInterval = setInterval(async () => {
        if (document.hidden) return; // Pausar se aba não está visível

        await updateClassesQuietly();
    }, 10000);
...
/**
 * Iniciar polling do modal (quando modal está aberto)
 */
function startModalPolling() {
    ...
    console.log('[Polling Modal] Iniciado - atualização a cada 5 segundos');

    // Polling para modal aberto (5 segundos - mais frequente)
    modalPollingInterval = setInterval(async () => {
        if (document.hidden) return; // Pausar se aba não está visível
        if (!currentClassId) return; // Parar se não tem turma selecionada
        ...
        await updateModalQuietly();
    }, 5000);
}

/**
 * Parar polling do modal
 */
function stopModalPolling() {
    if (modalPollingInterval) {
        clearInterval(modalPollingInterval);
        modalPollingInterval = null;
        console.log('[Polling Modal] Parado');
    }
}

/**
 * Atualizar turmas silenciosamente (sem loading excessivo)
 */
async function updateClassesQuietly() {
    try {
        const res = await fetch('../api/teacher/get_classes.php', {
            credentials: 'same-origin'
        });
        ...
        if (data.ok) {
            ...
            // Comparar com dados anteriores para ver se houve mudanças
            const hasChanges = !lastClassesData ||
                               JSON.stringify(lastClassesData) !== JSON.stringify(newClasses);

            if (hasChanges) {
                console.log('[Polling] Turmas atualizadas - mudanças detectadas');
                lastClassesData = newClasses;
                ...
                // Piscar indicador de atualização
                blinkPollingIndicator();
            }
        }
    } catch (err) {
        console.error('[Polling] Erro ao atualizar turmas:', err);
    }
}

/**
 * Atualizar modal silenciosamente (sem recarregar tudo)
 */
async function updateModalQuietly() {
    if (!currentClassId) return;

    try {
        // Atualizar contadores no cabeçalho do modal
        const resClasses = await fetch('../api/teacher/get_classes.php', {
            credentials: 'same-origin'
        });
        ...
        if (dataClasses.ok) {
            const cls = dataClasses.classes.find(c => c.id === currentClassId);
            if (cls) {
                const currentCount = parseInt(document.getElementById('modal-student-count').textContent);
                const currentPending = parseInt(document.getElementById('modal-pending-count').textContent);

                // Só atualizar se mudou
                if (currentCount !== cls.student_count || currentPending !== cls.pending_count) {
                    console.log('[Polling Modal] Contadores atualizados');
                    document.getElementById('modal-student-count').textContent = cls.student_count;
                    document.getElementById('modal-pending-count').textContent = cls.pending_count;
        ...
        console.error('[Polling Modal] Erro ao atualizar modal:', err);
    }
}

/**
 * Mostrar indicador visual de polling ativo
 */
function showPollingIndicator() {
    const header = document.querySelector('.teacher-header .header-left h1');
    if (!header) return;
    ...
    if (!document.getElementById('polling-indicator')) {
        ...
        indicator.innerHTML = '<i class="fas fa-sync-alt" style="font-size: 14px; color: #10b981; margin-left: 10px; opacity: 0.6;"></i>';
...
/**
 * Piscar indicador quando atualização ocorrer
 */
function blinkPollingIndicator() {
    const indicator = document.getElementById('polling-indicator');
    if (!indicator) return;
...
/**
 * Adicionar animação de spin no CSS (inline)
 */
(function addSpinAnimation() {
    if (document.getElementById('polling-spin-animation')) return;

    const style = document.createElement('style');
    style.id = 'polling-spin-animation';
    style.textContent = `
        @keyframes spin {
...
/**
 * Page Visibility API - Pausar/Retomar polling quando usuário muda de aba
 */
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        console.log('[Polling] Aba oculta - polling continua (com verificação)');
    } else {
        console.log('[Polling] Aba visível - forçando atualização imediata');
        // Atualizar imediatamente quando usuário volta
        if (isPollingActive) {
            updateClassesQuietly();
        }
        if (modalPollingInterval && currentClassId) {
            updateModalQuietly();
        }
    }
});
...
/**
 * Sobrescrever openClassModal para iniciar polling do modal
 */
const originalOpenClassModal = openClassModal;
openClassModal = async function(classId) {
    await originalOpenClassModal(classId);
    startModalPolling();
};

/**
 * Sobrescrever fechamento de modal para parar polling
 */
document.getElementById('close-class-modal')?.addEventListener('click', () => {
    document.getElementById('class-modal').classList.remove('active');
    stopModalPolling();
...
// Também parar polling ao clicar fora do modal
...
(async function init() {
    ...
    console.log('[Teacher] Dashboard inicializado com polling ativo');
})();
