 ...
 * Endpoint: POST /api/challenge/accept.php
 *
...
try {
    ...
    // Verificar se o desafio foi cancelado, completado ou já começou
    if ($participant['challenge_status'] === 'cancelled' || $participant['challenge_status'] === 'completed') {
        // Marcar como declined (convite inválido)
        $stmt = $pdo->prepare('
        ...
        $message = $participant['challenge_status'] === 'cancelled'
            ? 'Este desafio foi cancelado pelo professor'
            : 'Este desafio já foi finalizado';

        json_out(['ok' => false, 'error' => $message]);
    }

    // BUG FIX: Impedir aceitar após o desafio já ter começado
    if ($participant['challenge_status'] === 'active') {
        json_out(['ok' => false, 'error' => 'Este desafio já começou. Não é mais possível aceitar o convite.']);
    }
    ...
    error_log('Erro em accept.php: ' . $e->getMessage());
